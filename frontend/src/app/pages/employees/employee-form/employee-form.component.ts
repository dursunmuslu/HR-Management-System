...
import { Department, Team } from '../../../core/models/organization.model';
...
export class EmployeeFormComponent implements OnInit {
  ...
  readonly employeeForm = this.fb.nonNullable.group({
    ...
    team_id: [0, [Validators.required, Validators.min(1)]],
    position: [
      '',
  ...
  ngOnInit(): void {
    ...
    this.controls.department_id.valueChanges.subscribe(departmentId => {
      this.controls.team_id.setValue(0, { emitEvent: false });
      this.teams = [];
  ...
  get selectedDepartment(): Department | null {
    const departmentId = this.controls.department_id.value;
    return this.departments.find(d => d.id === departmentId) ?? null;
  }
  ...
  submit(): void {
    ...
    const selectedTeam = this.teams.find(t => t.id === value.team_id);

    if (!selectedTeam || !selectedTeam.is_active) {
      this.errorMessage = 'Geçerli ve aktif bir takım seçmelisiniz.';
      return;
    }
  ...
  private loadTeams(departmentId: number): void {
    ...
    this.organizationService
      ...
      .pipe(finalize(() => { this.isLoadingTeams = false; }))
      .subscribe({
        next: teams => {
          this.teams = teams
  ...
  private resolveError(error: HttpErrorResponse): string {
    if (error.status === 0) return 'Backend sunucusuna bağlanılamadı.';
    if (error.status === 409 && typeof error.error?.detail === 'string') return error.error.detail;
    if (error.status === 422 && Array.isArray(error.error?.detail)) {
      return error.error.detail
        .map((item: any) => `${item.loc?.at(-1) ?? 'alan'}: ${item.msg ?? 'Geçersiz değer'}`)
        .join(' | ');
    }
